...
class WatchGadget extends React.Component<{}, WatchState>
{
	...
	public render() 
	{
		return (
			<div>
				<AvStandardGrabbable modelUri={"./models/placeholder.glb"} modelScale={0.03}>
...
*  `<AvStandardGrabbable />`, a node that conveniently gives us a grabbable handle and a model all at once. IT's currently using the placeholder model for now (we'll be replacing it later), with its  scale adjusted. 
